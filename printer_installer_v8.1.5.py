...
def install_lan(ip, model, name, cb_log, t_fn=None, ask_generic_cb=None):
    import time as _time
    ...
    if not drv:
        cb_log(f"  [2/5] ⚠ '{model}' tapılmadı — avtomatik qeydiyyat başlayır…")
        auto_drv = auto_register_xprinter_driver(cb_log)
        if auto_drv:
            drv = auto_drv
        else:
            # Generic driver ilə davam etmək üçün istifadəçidən soruş
            use_generic = False
            if ask_generic_cb:
                use_generic = ask_generic_cb(model)
            if use_generic:
                generic = _find_generic_driver(cb_log)
                if generic:
                    drv = generic
                    cb_log(f"  [2/5] ℹ '{generic}' istifadə olunur")
                else:
                    cb_log(f"  ✗  Generic driver də tapılmadı!")
                    return False
            else:
                t = t_fn if t_fn else (lambda k, *a: k)
                cb_log(f"  ✗  Driver tapılmadı!")
                cb_log(t("no_driver_installed"))
                return False
...
class App(tk.Tk):
    ...
    def _lan_install(self):
        ...
        def run():
            ok = 0
            generic_answer = [None]  # None=soruşulmayıb, True/False=cavab
            def ask_generic(model):
                if generic_answer[0] is not None:
                    return generic_answer[0]
                evt = threading.Event()
                def _ask():
                    generic_answer[0] = messagebox.askyesno(
                        "Driver tapılmadı",
                        f"'{model}' driver tapılmadı.\n\n"
                        f"'Generic / Text Only' driver ilə davam edilsin?\n"
                        f"(Çek printerləri üçün işləyir)")
                    evt.set()
                self.after(0, _ask)
                evt.wait()
                return generic_answer[0]
            for ip, model, name in tasks:
                ...
                if install_lan(ip, model, name,
                               ...
                               t_fn=self.t, ask_generic_cb=ask_generic): ok += 1
            self.after(0, ov.destroy)
