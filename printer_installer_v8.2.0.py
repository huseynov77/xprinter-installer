...
LANGS = {
    "AZ": {
        ...
        "tab_info":"   Məlumat   ",
        "drv_choice_title":"Driver seçimi",
        "drv_choice_msg":"'{}' driver tapılmadı.\n\nHansı driver istifadə olunsun?",
        "drv_xp80c":"XP-80C (kəsicili XPrinter · yalnız 80mm)",
        "drv_generic":"Generic / Text Only (bütün ölçülər · bütün modellər)",
        "drv_xp80c_installing":"  XP-80C driver quraşdırılır (pnputil)…",
        "drv_xp80c_ok":"  ✓ XP-80C driver quraşdırıldı",
        "drv_xp80c_fail":"  ✗ XP-80C driver quraşdırıla bilmədi",
...
"team":"CloPos IT Team  ©2026",
    ...
    "EN": {
        ...
        "tab_info":"   Info   ",
        "drv_choice_title":"Driver Selection",
        "drv_choice_msg":"'{}' driver not found.\n\nWhich driver should be used?",
        "drv_xp80c":"XP-80C (XPrinter with cutter · 80mm only)",
        "drv_generic":"Generic / Text Only (all sizes · all models)",
        "drv_xp80c_installing":"  Installing XP-80C driver (pnputil)…",
        "drv_xp80c_ok":"  ✓ XP-80C driver installed",
        "drv_xp80c_fail":"  ✗ XP-80C driver installation failed",
...
"team":"CloPos IT Team  ©2026",
    ...
    "TR": {
        ...
        "tab_info":"   Bilgi   ",
        "drv_choice_title":"Sürücü Seçimi",
        "drv_choice_msg":"'{}' sürücü bulunamadı.\n\nHangi sürücü kullanılsın?",
        "drv_xp80c":"XP-80C (kesicili XPrinter · yalnız 80mm)",
        "drv_generic":"Generic / Text Only (tüm boyutlar · tüm modeller)",
        "drv_xp80c_installing":"  XP-80C sürücü kuruluyor (pnputil)…",
        "drv_xp80c_ok":"  ✓ XP-80C sürücü kuruldu",
        "drv_xp80c_fail":"  ✗ XP-80C sürücü kurulamadı",
...
"team":"CloPos IT Team  ©2026",
    ...
    "RU": {
        ...
        "tab_info":"   Инфо   ",
        "drv_choice_title":"Выбор драйвера",
        "drv_choice_msg":"Драйвер '{}' не найден.\n\nКакой драйвер использовать?",
        "drv_xp80c":"XP-80C (XPrinter с ножом · только 80мм)",
        "drv_generic":"Generic / Text Only (все размеры · все модели)",
        "drv_xp80c_installing":"  Установка XP-80C драйвера (pnputil)…",
        "drv_xp80c_ok":"  ✓ XP-80C драйвер установлен",
        "drv_xp80c_fail":"  ✗ Не удалось установить XP-80C драйвер",
...
def find_xprinter_driver(model):
    ...
    # Dəqiq model axtarışı (məs. "XP-80C")
    for d in drivers:
        if model.upper() in d.upper(): return d
    # Model ailə axtarışı (məs. "80" → "XP-80C" tapır, amma "350B" yox)
    model_num = ''.join(c for c in model if c.isdigit())
    if model_num:
        for d in drivers:
            if model_num in d and ("xp" in d.lower() or "xprinter" in d.lower()): return d
    return None

def _get_bundled_drv_dir():
    """Bundle olunmuş XPrinter POS driver fayllarının yolu"""
    base = sys._MEIPASS if getattr(sys,"frozen",False) \
           else os.path.dirname(os.path.abspath(__file__))
    return os.path.join(base, "xprinter_pos_drv")

def _find_generic_driver(cb_log=None):
    """Generic / Text Only driver tap, yoxdursa quraşdır"""
    _log = cb_log if cb_log else (lambda m: None)
    drivers = get_installed_drivers()
    for d in drivers:
        if "generic" in d.lower() and "text" in d.lower():
            return d
    # Generic driver yoxdursa, Add-PrinterDriver ilə əlavə et
    _log("  [auto-drv] Generic driver tapılmadı, əlavə edilir…")
    ok, out = ps_run(
...
def auto_register_xprinter_driver(cb_log=None):
    """
    Driver tapılmadıqda:
    1) Bundle olunmuş GPD + DLL ilə qeydiyyat cəhd edilir
    2) Uğursuz olarsa, "Generic / Text Only" driver fallback kimi istifadə olunur
       (bütün Windows versiyalarında mövcuddur, ESC/POS printerləri üçün işləyir)
    """
    _log = cb_log if cb_log else (lambda m: None)
    drv_name = "XP-80C"
    import time as _time, shutil, tempfile

    # Artıq varsa, skip (yalnız dəqiq model yoxla)
    drivers = get_installed_drivers()
    for d in drivers:
        if drv_name.upper() in d.upper():
            _log(f"  [auto-drv] Driver artıq var: {d}")
            return d

    _log(f"  [auto-drv] XPrinter driver tapılmadı, avtomatik qeydiyyat edilir…")

    src = _get_bundled_drv_dir()
    gpd = os.path.join(src, "XP80C.GPD")
    dll = os.path.join(src, "XP80.DLL")

    if not os.path.exists(gpd):
        _log(f"  [auto-drv] GPD fayl tapılmadı, Generic fallback yoxlanılır…")
    else:
        # Faylları spool qovluğuna kopyala
        spool = r"C:\Windows\System32\spool\drivers\x64\3"
        try:
            for f in [gpd, dll]:
                if os.path.exists(f):
                    dst = os.path.join(spool, os.path.basename(f))
                    if not os.path.exists(dst):
                        shutil.copy2(f, dst)
        except Exception:
            pass

        # AddPrinterDriverEx cəhd et
        ps1_path = os.path.join(tempfile.gettempdir(), "_clopos_add_drv.ps1")
        ps1 = (
            'Add-Type @"\n'
            'using System;\n'
            'using System.Runtime.InteropServices;\n'
            'public class PrnDrvLegacy {\n'
            ...
            '        string pName, uint Level, ref DRIVER_INFO_2 di, uint flags);\n'
            '}\n'
            '"@\n'
            f'$spool = "{spool}"\n'
            '$drv  = "$spool\\unidrv.dll"\n'
            '$data = "$spool\\XP80C.GPD"\n'
            '$cfg  = "$spool\\unidrvui.dll"\n'
            '$di = New-Object PrnDrvLegacy+DRIVER_INFO_2\n'
            '$di.cVersion     = 3\n'
            f'$di.pName        = "{drv_name}"\n'
            ...
            '$di.pDataFile    = $data\n'
            '$di.pConfigFile  = $cfg\n'
            '$ok = [PrnDrvLegacy]::AddPrinterDriverEx($null, 2, [ref]$di, 0)\n'
        ...
        # API ilə qeydiyyat uğurlu oldu?
        drivers = get_installed_drivers()
        for d in drivers:
            if drv_name.upper() in d.upper():
                _log(f"  [auto-drv] ✓ Driver qeydiyyat olundu: {d}")
                return d

    _log(f"  [auto-drv] ✗ XPrinter driver qeydiyyat olunmadı")
    return None

def get_bundled_exe():
    """Legacy — artıq istifadə olunmur (V7.77 silindi)"""
    return ""
...
def run_install_exe(exe_path, cb_log, t_fn=None):
    ...
    # Installer-dən sonra driveri qeydiyyat etməyə çalış
    import time as _t; _t.sleep(2)
    drv = find_xprinter_driver("XP-80C")
    if not drv:
        cb_log("  Driver qeydiyyat edilir…")
        auto_register_xprinter_driver(cb_log)
    return True

def _ensure_spooler(cb_log=None):
    """Print Spooler xidmetinin islediyine emin ol (Win10 fix)"""
    import time as _time
    _log = cb_log if cb_log else (lambda m: None)
    chk, out = ps_run("(Get-Service -Name Spooler).Status", timeout=10)
    _log(f"  [Spooler] status: {out.strip()}")
    if chk and "Running" in out:
        return True
    _log("  [Spooler] işləmir, start edilir…")
    ps_run("Start-Service -Name Spooler -EA SilentlyContinue", timeout=10)
    for i in range(5):
        _time.sleep(1)
        chk, out = ps_run("(Get-Service -Name Spooler).Status", timeout=10)
        _log(f"  [Spooler] gözlənilir ({i+1}/5)… status: {out.strip()}")
        if chk and "Running" in out:
            return True
    return False

def _install_xp80c_pnputil(cb_log, t_fn=None):
    """pnputil ilə bundled XP-80C INF driverini quraşdır"""
    t = t_fn if t_fn else (lambda k, *a: k)
    cb_log(t("drv_xp80c_installing"))
    drv_dir = _get_bundled_drv_dir()
    inf = os.path.join(drv_dir, "XP80C.inf")
    if not os.path.exists(inf):
        cb_log(f"  ✗ INF tapılmadı: {inf}")
        return None
    try:
        r = subprocess.run(
            ...
            check=False, creationflags=NO_WIN)
        out = (r.stdout + r.stderr).strip()
        cb_log(f"  pnputil: {out[:200]}")
    except Exception as e:
        cb_log(f"  ✗ pnputil xəta: {e}")
        return None
    import time as _t; _t.sleep(2)
    # pnputil-dən sonra driveri yoxla
    drv = find_xprinter_driver("XP-80C")
    if drv:
        cb_log(t("drv_xp80c_ok"))
        return drv
    # pnputil uğursuz — manual register cəhd et
    auto_drv = auto_register_xprinter_driver(cb_log)
    if auto_drv:
        cb_log(t("drv_xp80c_ok"))
        return auto_drv
    cb_log(t("drv_xp80c_fail"))
    return None

def _resolve_driver(model, cb_log, t_fn=None):
    """Model seçiminə görə driver tap/quraşdır.
    model = "XP-80C" → XP-80C driver, tapılmasa pnputil ilə quraşdır
    model = "Generic" → Generic / Text Only driver
    """
    if model == "Generic":
        generic = _find_generic_driver(cb_log)
        if generic:
            cb_log(f"  ℹ '{generic}' istifadə olunur")
            return generic
        cb_log(f"  ✗ Generic driver tapılmadı!")
        return None
    # XP-80C və ya digər model
    drv = find_xprinter_driver(model)
    if drv:
        return drv
    cb_log(f"  ⚠ '{model}' tapılmadı — avtomatik qeydiyyat başlayır…")
    auto_drv = auto_register_xprinter_driver(cb_log)
    if auto_drv:
        return auto_drv
    # pnputil ilə quraşdır
    xp_drv = _install_xp80c_pnputil(cb_log, t_fn)
    if xp_drv:
        return xp_drv
    cb_log("  ⚠ XP-80C alınmadı, Generic fallback…")
    generic = _find_generic_driver(cb_log)
    if generic:
        cb_log(f"  ℹ '{generic}' istifadə olunur")
        return generic
    cb_log(f"  ✗ Heç bir driver tapılmadı!")
    return None

def install_lan(ip, model, name, cb_log, t_fn=None):
    import time as _time
    pn = f"IP_{ip.replace('.','_')}"
    cb_log(f"\n  ── LAN Install: {ip} → {name} [{model}] ──")

    # 1. Spooler yoxla
    cb_log("  [1/5] Spooler yoxlanılır…")
    if not _ensure_spooler(cb_log):
        cb_log("  ⚠  Spooler işləmir, restart edilir…")
        ps_run("Restart-Service -Name Spooler -Force -EA SilentlyContinue", timeout=15)
        _time.sleep(3)
        if not _ensure_spooler(cb_log):
            cb_log("  ✗  Spooler başladıla bilmədi!")
            return False

    # 2. Driver tap
    cb_log(f"  [2/5] Driver axtarılır: {model}…")
    drv = _resolve_driver(model, cb_log, t_fn)
    if not drv:
        return False
    cb_log(f"  [2/5] ✓ Driver tapıldı: {drv}")
    ...
    if ok_port:
        ...
        cb_log(f"  [3/5] ⚠  Port əmri xətası: {err_port}")

    # 4. Port yaranmasını gözlə
    cb_log(f"  [4/5] Port mövcudluğu yoxlanılır…")
    port_ok = False
    for i in range(6):
        chk, chk_out = ps_run(f"Get-PrinterPort -Name '{pn}' -EA Stop", timeout=10)
        if chk:
            cb_log(f"  [4/5] ✓ Port mövcuddur")
            port_ok = True
            break
        cb_log(f"  [4/5] gözlənilir ({i+1}/6)… {chk_out[:80]}")
        _time.sleep(1)
    if not port_ok:
        cb_log(f"  ✗  Port '{pn}' yaranmadı — quraşdırma dayandırıldı")
        return False

    # 5. Printer yarat
    cb_log(f"  [5/5] Printer yaradılır: '{name}'…")
    ok, err = ps_run(
        ...
        timeout=45)
    if ok:
        cb_log(f"  [5/5] ✓ Printer əmri uğurlu")
    else:
        cb_log(f"  [5/5] ⚠  Printer əmri cavabı: {err}")

    # Doğrulama
    if not ok:
        cb_log("  [doğrulama] 2 san gözlənilir…")
        _time.sleep(2)
        chk2, chk2_out = ps_run(f"Get-Printer -Name '{name}' -EA Stop", timeout=10)
        if chk2:
            cb_log(f"  [doğrulama] ✓ Printer tapıldı (əmr xəta versə də yaranıb)")
            ok = True
        else:
            cb_log(f"  [doğrulama] ✗ Printer tapılmadı: {chk2_out}")

    if ok: cb_log(f"  ✓  '{name}' uğurla quraşdırıldı")
    else:  cb_log(f"  ✗  '{name}' quraşdırıla bilmədi: {err}")
    return ok
...
class App(tk.Tk):
    ...
    def _notebook(self):
        ...
        for tab, key in [(self.t2,"tab_lan"),
    ...
    def _lang_ch(self, *_):
        for i, key in enumerate(["tab_lan","tab_usb","tab_list","tab_info"]):
    ...
    # ══════════════════════════════════════════════════════════════
    #  TAB 1 — LAN  (scrollable, install button inside)
    # ══════════════════════════════════════════════════════════════
    def _build_t2(self):
        f = self.t2
    ...
    def _model_sel(self, parent, var):
        ...
        choices = [
            ("XP-80C", self.t("drv_xp80c"), CL_BLUE, CL_WHITE),
            ("Generic", self.t("drv_generic"), CL_CARD2, CL_SUB),
        ]
        for val, label, bg, fg in choices:
            row = tk.Frame(mf, bg=CL_CARD, pady=3)
            ...
            tk.Label(row, text=label, font=("Segoe UI",8),
                     bg=bg, fg=fg,
                     padx=8, pady=4, wraplength=190,
                     justify="left").pack(side="left", padx=(2,0))
