...
def auto_register_xprinter_driver(cb_log=None):
    ...
    if generic:
        _log(f"  [auto-drv] XPrinter qeydiyyat olmadı — '{generic}' istifadə olunacaq")
        _log(f"  [auto-drv] (Bu driver çek printerləri üçün işləyir)")
        return generic

    _log(f"  [auto-drv] ✗ Heç bir uyğun driver tapılmadı")
    return None
...
def install_lan(ip, model, name, cb_log, t_fn=None):
    ...
    drivers = get_installed_drivers()
    cb_log(f"  [2/5] Quraşdırılmış driverlər ({len(drivers)}): {', '.join(drivers[:5])}")
    ...
    if not drv:
        ...
        if auto_drv:
            ...
            t = t_fn if t_fn else (lambda k, *a: k)
            cb_log(f"  ✗  Driver avtomatik qeydiyyat olunmadı!")
